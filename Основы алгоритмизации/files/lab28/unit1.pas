...
begin
  ...
  L3:=StrToInt(Edit4.Text);
  k2:=StrToInt(Edit5.Text);
end;

procedure TForm1.PaintBox1Paint(Sender: TObject);
var
  x, y, xc, yc: integer;
  Q, angle1, angle2, angle3: double;
begin
  Q:=pi/180;
  with PaintBox1 do
  begin
  ...
  angle2:=angle1*(1 + k);
  angle3:=angle1*(1 + k2);
  x:=round(L1*cos(Q*angle1)+L2*cos(Q*angle2)+L3*cos(Q*angle3));
  y:=-round(L1*sin(Q*angle1)+L2*sin(Q*angle2)+L3*sin(Q*angle3));
  if angle1=0
